...
def test_created_datafile_exists(raster_dataset):
    assert os.path.isfile(raster_dataset.filename) 

def test_dataset_format_is_geotiff(gdal_dataset):
...
def test_dataset_height_in_pixels_is_4(gdal_dataset):
    assert gdal_dataset.RasterYSize == DATASET_ROW_COUNT

def test_dataset_width_in_pixels_is_5(gdal_dataset):
    assert gdal_dataset.RasterXSize == DATASET_COLUMN_COUNT

def test_dataset_band_count_is_6(gdal_dataset):
...
def test_projection_is_wgs84(gdal_dataset):
